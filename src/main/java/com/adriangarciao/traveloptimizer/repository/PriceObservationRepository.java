...
public interface PriceObservationRepository extends JpaRepository<PriceObservation, UUID> {
    ...
    /**
     * Find recent price observations for a route, ordered by creation time descending.
     * Uses a time window to avoid including very old data.
     */
    @Query("SELECT p FROM PriceObservation p WHERE p.origin = :origin AND p.destination = :destination " +
    ...
    /**
     * Find all price observations for a route (any departure date within range).
     * Useful for routes where exact date match may not have enough history.
     */
    @Query("SELECT p FROM PriceObservation p WHERE p.origin = :origin AND p.destination = :destination " +
     ...
     */
    @Query("SELECT COUNT(p) FROM PriceObservation p WHERE p.origin = :origin AND p.destination = :destination " +
